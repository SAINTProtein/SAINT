...
from time import time
t0 = time()
from GenerateForWindow10 import generateDatasetWindow10
from GenerateForWindow20 import generateDatasetWindow20
from GenerateForWindow50 import generateDatasetWindow50
...
cw10 = [None] # generateDatasetWindow10(inputlist,inputdir)
cw20 = [None] # generateDatasetWindow20(inputlist,inputdir)
cw50 = [None] # generateDatasetWindow50(inputlist,inputdir)
...
if True:
    ...
    t2 = Thread(target=generateDatasetWindow10, args=(cw10, inputlist, inputdir)) 
    t3 = Thread(target=generateDatasetWindow20, args=(cw20, inputlist, inputdir)) 
    t4 = Thread(target=generateDatasetWindow50, args=(cw50, inputlist, inputdir))
    ...
    generateDatasetWindow10(cw10, inputlist,inputdir)
    generateDatasetWindow20(cw20, inputlist,inputdir)
    generateDatasetWindow50(cw50, inputlist,inputdir)
    generateLengthlist(lengths, inputlist,inputdir)


# print(base_dataset[0].shape, cw10[0].shape, cw20[0].shape, cw50[0].shape, lengths[0][1].shape, time()-t0)
...
features10 = np.concatenate([base_dataset[0], cw10[0]], axis=-1)
features20 = np.concatenate([base_dataset[0], cw20[0]], axis=-1)
features50 = np.concatenate([base_dataset[0], cw50[0]], axis=-1)
...
t2 = time()
ss8_win0 = model.predict([features0, attention_mask, pos_ids], batch_size=config.predict_batch_size)
...
t0 = time()
model = get_model(num_feature=73)
print('Model_creation_time:', time()-t0)

t1 = time()
model.load_weights('./SAINT_win10_weights.h5')
print('Weights_loading_time:', time()-t1)
...
t2 = time()
ss8_win10 = model.predict([features10, attention_mask, pos_ids], batch_size=config.predict_batch_size)
print('Inference_time:', time()-t2)
...
t0 = time()
model = get_model(num_feature=93)
print('Model_creation_time:', time()-t0)

t1 = time()
model.load_weights('./SAINT_win20_weights.h5')
print('Weights_loading_time:', time()-t1)
...
t2 = time()
ss8_win20 = model.predict([features20, attention_mask, pos_ids], batch_size=config.predict_batch_size)
print('Inference_time:', time()-t2)
...
t0 = time()
model = get_model(num_feature=153)
print('Model_creation_time:', time()-t0)

t1 = time()
model.load_weights('./SAINT_win50_weights.h5')
print('Weights_loading_time:', time()-t1)
...
t2 = time()
ss8_win50 = model.predict([features50, attention_mask, pos_ids], batch_size=config.predict_batch_size)
print('Inference_time:', time()-t2)
...
# for i in range(2,11):
#   print(model.get_layer('my_layer_{}'.format(i)).get_weights())
t000 = time()
avg = ss8_win0 + ss8_win10 + ss8_win20 + ss8_win50
avg /= 4
...
# t000 = time()
ss8_ = np.argmax(avg, axis=-1)
ss8_win0_ = np.argmax(ss8_win0, axis=-1)
ss8_win10_ = np.argmax(ss8_win10, axis=-1)
ss8_win20_ = np.argmax(ss8_win20, axis=-1)
...
for i in range(avg.shape[0]):
    if i in greater_than_700_dict:
        ...
        temp_ss8 = np.zeros([greater_than_700_dict[i][1], avg.shape[2]])
        temp_ss8_win0 = np.zeros([greater_than_700_dict[i][1], avg.shape[2]])
        temp_ss8_win10 = np.zeros([greater_than_700_dict[i][1], avg.shape[2]])
        temp_ss8_win20 = np.zeros([greater_than_700_dict[i][1], avg.shape[2]])
        temp_ss8_win50 = np.zeros([greater_than_700_dict[i][1], avg.shape[2]])
        
        temp_ss8[:700, :] = avg[i]
        temp_ss8_win0[:700, :] = ss8_win0[i]
        temp_ss8_win10[:700, :] = ss8_win10[i]
        ...
        temp_ss8_win10 = np.argmax(temp_ss8_win10, axis=-1)
        temp_ss8_win20 = np.argmax(temp_ss8_win20, axis=-1)
        temp_ss8_win50 = np.argmax(temp_ss8_win50, axis=-1)
        
        print('temp_ss8_win50.shape:', temp_ss8_win50.shape)
        for j in range(temp_ss8_win50.shape[0]):
            ss8_string += _structures_[temp_ss8[j]]
            ss8_win0_string += _structures_[temp_ss8_win0[j]]
            ss8_win10_string += _structures_[temp_ss8_win10[j]]
            ss8_win20_string += _structures_[temp_ss8_win20[j]]
            ss8_win50_string += _structures_[temp_ss8_win50[j]]
        ss8_string += '\n'
        ss8_win0_string += '\n'
        ss8_win10_string += '\n'
        ss8_win20_string += '\n'
    ...
    for j in range(int(lengths[0][0][i])):
        ss8_string += _structures_[ss8_[i, j]]
...
with open('outputs/SAINT_cwin10_output_ss8_sequences.txt', 'w') as f:
    f.write(ss8_win10_string)
    
with open('outputs/SAINT_cwin20_output_ss8_sequences.txt', 'w') as f:
    f.write(ss8_win20_string)
